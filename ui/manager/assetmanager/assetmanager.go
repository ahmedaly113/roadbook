...
func (am *AssetManager) RenderText(surfaceKey, fontKey string, text string, color sdl.Color, hicolor sdl.Color) (*sdl.Surface, error) {
	surface, err := util.RenderText(am.Fonts[fontKey], text, color, hicolor)
...
func (am *AssetManager) GetFont(fontKey string) *ttf.Font {
	return am.Fonts[fontKey]
}
...
func (am *AssetManager) renderJSONText(jsonFile string, data interface{}) {
	...
	for _, v := range textArray {
		...
		bgRgba, ok := textInfo["bgRgba"].([]interface{})
		if len(bgRgba) == 4 {
			bgColor, err = sdlColorFromInterfaceArray(bgRgba)
		}
		if !ok || len(bgRgba) != 4 || err != nil {
			loadJSONPanic(jsonFile, id, "bgRgba needs to be in form [R,G,B,A], 0-255 for each element")
		}

		if _, err = am.RenderText(id, font, text, *color, *bgColor); err != nil {
